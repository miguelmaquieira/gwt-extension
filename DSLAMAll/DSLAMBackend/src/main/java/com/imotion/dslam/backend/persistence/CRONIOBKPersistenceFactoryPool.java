...
public class CRONIOBKPersistenceFactoryPool extends AEMFTPersistenceFactoryPool implements CRONIOBKIPersistenceFactory {
	...
	public CRONIOBKILogNodePersistenceService newLogNodePersistence(String sessionId) {
		String impl = CRONIOBKIPersistenceConstants.CTE_CRONIO_PERSISTENCE_LOGNODE_PERSISTENCE_DEFAULT_IMPL;
		if (getConfigSrv() != null) {
			impl = getConfigSrv().getProperty(
					CRONIOBKIPersistenceConstants.CFG_CRONIO_PERSISTENCE_LOGNODE_PERSISTENCE_IMPL,
					CRONIOBKIPersistenceConstants.CTE_CRONIO_PERSISTENCE_LOGNODE_PERSISTENCE_DEFAULT_IMPL);
		}
		return (CRONIOBKILogNodePersistenceService) newPersistenceService(impl, sessionId);
	}

...
public class CRONIOBusDesktopScheduleList extends AEGWTBootstrapTable {

	public static final String NAME = "CRONIOBusDesktopScheduleList";
	private static CRONIOBusI18NTexts TEXTS = GWT.create(CRONIOBusI18NTexts.class);

	public CRONIOBusDesktopScheduleList(AEGWTButton deleteButton) {
		...
		if (data != null) {
			...
			for (AEMFTMetadataElement scheduleData : scheduleList) {
				...
				if (!CRONIOBOIProjectDataConstants.INFO.equals(itemKey)) {
					...
					String 						formattedDate 		= AEGWTBusinessUtils.getFormattedTimeMessage(scheduleValue, CRONIOBusDesktopProcessConfigureScheduleForm.DATE_FORMAT);

...
public class CRONIOBusDesktopProcessConfigureSchedule extends AEGWTCompositePanel implements AEGWTHasLogicalEventHandlers {

	public static final String NAME = "CRONIOBusDesktopProcessConfigureSchedule";

	private FlowPanel 										root;
	private FlowPanel 										scheduleListZone;
	private	 CRONIOBusDesktopScheduleList    				scheduleList;
	private CRONIOBusDesktopProcessConfigureScheduleForm	scheduleForm;
	private	 AEMFTMetadataElementComposite					schedulesData;
	private boolean 										addJS;							

	public CRONIOBusDesktopProcessConfigureSchedule() {
		...
		scheduleListZone.addStyleName(CRONIOBusDesktopIStyleConstants.PROCESS_CONFIGURE_SCHEDULE_LIST);
		root.add(scheduleListZone);
		scheduleList = new CRONIOBusDesktopScheduleList(header.getDeleteButton());
	...
	public void dispatchEvent(AEGWTLogicalEvent evt) {
		if (CRONIOBusDesktopProcessConfigureScheduleForm.NAME.equals(evt.getSourceWidget()) && LOGICAL_TYPE.SAVE_EVENT.equals(evt.getEventType())) {
		...
		} else if(CRONIOBusDesktopScheduleList.NAME.equals(evt.getSourceWidget()) && LOGICAL_TYPE.EDIT_EVENT.equals(evt.getEventType())) {
		...
		} else 	if(CRONIOBusDesktopScheduleList.NAME.equals(evt.getSourceWidget()) && LOGICAL_TYPE.DELETE_EVENT.equals(evt.getEventType())) {
	...
	private void addSchedules(String originalSchedule, String newSchedule) {
		...
		Date scheduleDate = AEGWTBusinessUtils.getDateFromFormattedTime(newSchedule, CRONIOBusDesktopProcessConfigureScheduleForm.DATE_FORMAT);
		schedulesData.addElement(newSchedule, scheduleDate);
		scheduleList.setData(schedulesData);
		getSchedulePopup().resetForm();	
	}
	
	private CRONIOBusDesktopProcessConfigureScheduleForm getSchedulePopup() {
		if (scheduleForm == null) {
			scheduleForm = new CRONIOBusDesktopProcessConfigureScheduleForm(this);	
		}
		return scheduleForm;
	}
}

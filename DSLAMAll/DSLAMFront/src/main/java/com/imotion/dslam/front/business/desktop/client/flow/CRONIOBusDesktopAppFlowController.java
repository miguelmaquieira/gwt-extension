...
import com.imotion.dslam.front.business.desktop.client.CRONIOBusDesktopHistoryNavigationConstants;
...
public class CRONIOBusDesktopAppFlowController extends AEGWTBaseFlowController {

	public static final String NAME = "BusinessDesktopManagementAppFlowController";

	public CRONIOBusDesktopAppFlowController(AEGWTIPresenter presenter, AEGWTCompositePanelViewDisplay display) {
		super(display);
		...
		if (super.active) {
			...
			if (!processed) {
				if (AEGWTLoginPresenterConstants.NAME.equals(sourceWindow)) {
					...
					History.newItem(CRONIOBusDesktopHistoryNavigationConstants.TOKEN_PROJECT_PAGE);
				}
			}
		}
	}
	...
	private void preferencesFlowEvent() {
		History.newItem(CRONIOBusDesktopHistoryNavigationConstants.TOKEN_PREFERENCES);
	}
	
	private void machinesFlowEvent() {
		History.newItem(CRONIOBusDesktopHistoryNavigationConstants.TOKEN_MACHINES);
	}
	
	private void userFlowEvent() {
		History.newItem(CRONIOBusDesktopHistoryNavigationConstants.TOKEN_USER);
	}
	...
	private void projectProcessFlowEvent(AEGWTFlowEvent evt) {
		...
		if (CRONIOBusProjectBasePresenterConstants.SECTION_TYPE_PROCESS.equals(mainSectionId) ||CRONIOBusProjectBasePresenterConstants.SECTION_TYPE_ENVIROMENTS.equals(mainSectionId)) {
			History.newItem(CRONIOBusDesktopHistoryNavigationConstants.TOKEN_PROCESS_PAGE);
		} else if (CRONIOBusProjectBasePresenterConstants.SECTION_TYPE_SCRIPT.equals(mainSectionId)) {
			History.newItem(CRONIOBusDesktopHistoryNavigationConstants.TOKEN_SCRIPTS_MANAGER);
		} else if (CRONIOBusProjectBasePresenterConstants.SECTION_TYPE_EXECUTION.equals(mainSectionId)) {
			History.newItem(CRONIOBusDesktopHistoryNavigationConstants.TOKEN_EXECUTION);
		} else if (CRONIOBusProjectBasePresenterConstants.SECTION_TYPE_LOG.equals(mainSectionId)) {
			History.newItem(CRONIOBusDesktopHistoryNavigationConstants.TOKEN_LOG);
		} else {
			History.newItem(CRONIOBusDesktopHistoryNavigationConstants.TOKEN_PROJECT_PAGE);
		}
	}

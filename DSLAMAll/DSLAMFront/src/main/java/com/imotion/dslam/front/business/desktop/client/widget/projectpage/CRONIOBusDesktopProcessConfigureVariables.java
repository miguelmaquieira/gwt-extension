...
public class CRONIOBusDesktopProcessConfigureVariables extends AEGWTCompositePanel implements AEGWTHasLogicalEventHandlers {

	public static final String NAME = "CRONIOBusDesktopProcessConfigureVariables";
	private static CRONIOBusI18NTexts TEXTS = GWT.create(CRONIOBusI18NTexts.class);
	
	private FlowPanel 											root;
	private FlowPanel											variableListZone;
	private	 CRONIOBusDesktopVariablesList   					variableList;
	private CRONIOBusDesktopProcessConfigureVariablesForm		variablesForm;
	private	 AEMFTMetadataElementComposite						variablesData;

	public CRONIOBusDesktopProcessConfigureVariables() {
		root = new FlowPanel();
		initWidget(root);
		...
		variableListZone.addStyleName(CRONIOBusDesktopIStyleConstants.PROCESS_CONFIGURE_VARIABLES_LIST);
		root.add(variableListZone);
		variableList = new CRONIOBusDesktopVariablesList(header.getDeleteButton());
	...
	public void postDisplay() {
		super.postDisplay();
		variablesForm = new CRONIOBusDesktopProcessConfigureVariablesForm(this);
	...
	public void dispatchEvent(AEGWTLogicalEvent evt) {
		if (CRONIOBusDesktopProcessConfigureVariablesForm.NAME.equals(evt.getSourceWidget()) && LOGICAL_TYPE.SAVE_EVENT.equals(evt.getEventType())) {
		...
		} else if(CRONIOBusDesktopVariablesList.NAME.equals(evt.getSourceWidget()) && LOGICAL_TYPE.EDIT_EVENT.equals(evt.getEventType())) {
		...
		} else if(CRONIOBusDesktopVariablesList.NAME.equals(evt.getSourceWidget()) && LOGICAL_TYPE.DELETE_EVENT.equals(evt.getEventType())) {

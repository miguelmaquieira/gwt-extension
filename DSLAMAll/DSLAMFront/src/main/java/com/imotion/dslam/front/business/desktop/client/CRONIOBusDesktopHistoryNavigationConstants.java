package com.imotion.dslam.front.business.desktop.client;


public interface CRONIOBusDesktopHistoryNavigationConstants {

	public final static String TOKEN_SCRIPTS_MANAGER 	= "scriptsManager";
	public final static String TOKEN_PROJECT_PAGE 	= "projectPage";
	public final static String TOKEN_PROCESS_PAGE 	= "processPage";
	public final static String TOKEN_EXECUTION 		= "execution";
	public final static String TOKEN_PREFERENCES 		= "preferences";
	public final static String TOKEN_MACHINES 		= "machines";
	public final static String TOKEN_USER 			= "user";
	public final static String TOKEN_LOG 				= "log";

}
